var TrysteroDiscovery = (() => {
  ...
  var getRelays = (config, defaults2, defaultN, deriveFromAppId = false) => config.relayConfig?.urls || (deriveFromAppId ? shuffle(defaults2, strToNum(config.appId)) : defaults2).slice(0, config.relayConfig?.redundancy ?? defaultN);
  ...
  var relayManager2 = createRelayManager((client) => client.socket);
  var topicToInfoHash = {};
  var infoHashToTopic = {};
  var announceIntervals = relayManager2.scoped();
  var announceFns = relayManager2.scoped();
  var subscriptionTokens = relayManager2.scoped();
  var trackerAnnounceMs = {};
  var handledSignals = {};
  var msgHandlers2 = relayManager2.scoped();
  var topicStates = relayManager2.scoped();
  ...
  var joinRoomStrategy = strategy_default({
    init: (config) => getRelays(config, defaultRelayUrls2, defaultRedundancy2).map((rawUrl) => {
      const client = relayManager2.register(rawUrl, () => makeSocket(rawUrl, (rawData) => {
        ...
        if ((data.offer || data.answer) && topic && data.offer_id) {
          ...
          msgHandlers2.forKey(rawUrl)[topic]?.(data);
        }
      }));
      return client.ready;
    }),
  ...
  var joinRoom2 = (config, roomId, callbacks) => joinRoomStrategy({
    ...config,
  ...
  var TrysteroDiscovery = class {
    ...
    async start({ network, strategy, relayUrls }, { onMessage, onPeer } = {}) {
      const join = strategy === "torrent" ? joinRoom2 : joinRoom;
      const selfId2 = strategy === "torrent" ? selfId : selfId;
    ...
    async publish(_channel, msgObj) {
      if (this._send) await this._send(msgObj);
    }
    async stop() {
      if (this.room) {
        try {
